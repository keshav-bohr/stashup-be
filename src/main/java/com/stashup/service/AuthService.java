...
import com.stashup.entity.RefreshToken;
...
public class AuthService {
  ...
  public TokenPair login(LoginRequest request) {
    ...
    if (!passwordEncoder.matches(request.password(), user.getPasswordHash())) {
      // Committed separately: this method ends by throwing, which would otherwise roll the
      // increment back and leave the account permanently unlockable.
      lockout.recordFailure(user.getId());
      throw new ApiException(ErrorCode.INVALID_CREDENTIALS);
    }
